...
old.cardsTable           = {}
old.rtGraphTable         = {}
old.commanderStats       = nil
old.killGraphTable       = {}
old.buildingSummaryTable = {}
old.statusSummaryTable   = {}
old.techLogTable         = {}
...
local function InitEndStats(self)
	...
	for _, message in ipairs(finalStatsTable) do
		...
		table.insert(teamObj.playerRows, CreateScoreboardRow(
			...
			message.accuracyOnos == -1 and string.format("%s%%", printNum(message.accuracy)) or string.format("%s%% (%s%%)",
			printNum(message.accuracy),
			printNum(message.accuracyOnos)),
			printNum(message.pdmg),
			printNum(message.sdmg),
			string.format("%d:%02d", minutes, seconds),
			string.format("%d:%02d", pMinutes, pSeconds),
			message.minutesComm > 0 and string.format("%d:%02d", cMinutes, cSeconds) or nil,
			message.steamId, message.isRookie, message.hiveSkill
		))
	...
	local teamStatsVisible = gameInfo and gameInfo.showEndStatsTeamBreakdown or true
	...
	old.finalStatsTable      = finalStatsTable
	old.avgAccTable          = avgAccTable
	old.miscDataTable        = miscDataTable
	old.cardsTable           = cardsTable
	old.rtGraphTable         = rtGraphTable
	old.commanderStats       = commanderStats
	old.killGraphTable       = killGraphTable
	old.buildingSummaryTable = buildingSummaryTable
	old.statusSummaryTable   = statusSummaryTable
	old.techLogTable         = techLogTable
...
Script.AddShutdownFunction(function()
	...
	local file = io.open(lastRoundFile, "w+")

	file:write(json.encode {
		finalStatsTable      = old.finalStatsTable,
		avgAccTable          = old.avgAccTable,
		miscDataTable        = old.miscDataTable,
		cardsTable           = old.cardsTable,
		rtGraphTable         = old.rtGraphTable,
		commanderStats       = old.commanderStats,
		killGraphTable       = old.killGraphTable,
		buildingSummaryTable = old.buildingSummaryTable,
		statusSummaryTable   = old.statusSummaryTable,
		techLogTable         = old.techLogTable,
	})
